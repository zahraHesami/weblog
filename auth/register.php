...
$error ='';
 if (
         isset($_POST['email']) && isset($_POST['password']) && isset($_POST['confirm']) && isset($_POST['first_name']) && isset($_POST['last_name'])
       && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['confirm']) && !empty($_POST['first_name']) && !empty($_POST['last_name']))
     {
           if($_POST['password'] === $_POST['confirm'])
           {
               if(strlen($_POST['password']) >5)
               {
                   ...
                   if(!$user)
                   {
                         $query='INSERT INTO php_project.users (email, password, first_name, last_name,created_at) VALUES (?, ?, ?, ?,NOW())';
                         $statement = $pdo->prepare($query);
                         $password = password_hash($_POST['password'], PASSWORD_DEFAULT);
                         $statement->execute([$_POST['email'],$password , $_POST['first_name'], $_POST['last_name']]);
 ...
 {
     if(!empty($_POST))
         $error='you need to fill all the required fields';
 }
?>
    ...
    <section style="height: 100vh; background-color: #138496;" class="d-flex justify-content-center align-items-center">
        <section style="width: 20rem;">
            ...
            <section class="bg-light my-0 px-2">
                <small class="text-danger"> <?php if(!empty('error')){ echo $error;} ?></small>
            </section>
